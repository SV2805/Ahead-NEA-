...
class HomePage:
  ...
  def create_deck(self):
    ...
    # selected_deck = easygui.buttonbox("Select a category in which you want to create deck:", choices=options)
  ...
  def get_display_decks(self,frame):
    # deck_btn_frame = tk.Frame(frame,height=200,background="white").pack(expand=True,fill="x")
    def get_display_v2():
      for widget in frame.winfo_children():
            widget.destroy()
    ...
    #create buttons for decks operation
    self.qs,self.ans = "",""
    def show_deck():
        frame = tk.Tk()
